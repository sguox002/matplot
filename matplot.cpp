...
void MatPlot::setup_figure()
{
	...
	figure();//no mode1
	layer();//no mode 1
	//axes();//mode 1 reset //not necessary to create two axes by sguo
	//mytimer.record_currtime();
	DISPLAY();
	mode=1;
}

//mode=1: update the figures
void MatPlot::update_figure()
{
	mode=1;
	...
	//axes();//not necessary to create two axes by sguo
	//mytimer.record_currtime();
	DISPLAY();
...
void MatPlot::display()
{
    ...
    while(!is_done)
	{
		...
		{// Configuration of objects
			if(is_debug1){cout<<"============================= configuration ..."<<endl;}
			//removed by sguo, which is not necessary to do it twice
			//printf("mode 1\n");
...
void MatPlot::display_figure()
{
    ...
    if(cf->Visible)
	{
		...
		{
			tObj=cf->Children[i]%100; //type of object
			iObj=cf->Children[i]/100; //number of object
...
int MatPlot::layer()
{
    ...
    axes();//default axes: in case no function created the axes using this one
...
void MatPlot::display_layer2()
{
    ...
    if(xPassive<25)
	{
		...
		//glOrtho( 0, (int)(window_w), (int)(window_h),0,-1,3);
		glDisable(GL_LINE_STIPPLE); 
		//gl2psDisable(GL2PS_LINE_STIPPLE); //by sguo
		...
		for(int j=0;j<vLayer.size();++j)
		{
			...
			//gl2psText(s.c_str(), "Arial", 12); //by sguo
...
void MatPlot::display_axes_2d()
{
    ...
    glViewport(0,0, (int)(window_w),(int)(window_h));
    glLoadIdentity();
    //gluOrtho2D( 0.0, 1.0, 0.0, 1.0 );//by sguo
    ...
    if(ca->Box)
	{
		...
		//gl2psLineWidth(ca->LineWidth); //by sguo
		if(ca->Selected)
		{
		...
		if(ca->XGrid)
		{
			...
			for(int i=0;i<ca->XTick.size();++i)
			{
				...
				{//- -
					...
					//gl2psEnable(GL2PS_LINE_STIPPLE); //by sguo
				}
				if(gridlinestyle==3)
				{//:		    
					glEnable(GL_LINE_STIPPLE);
...
int MatPlot::line()
{
    ...
    if(mode==1)
	{
		//if(iLine<vLine.size()){ vLine[iLine].reset(); } //do not have to reset it!
    }
...
int MatPlot::line(const dvec& x,const dvec& y)
{
    int h=line();
    if(cfr->Visible)
	{
...
int MatPlot::line(const dvec& x,const dvec& y,const dvec& z)
{
    int h=line();
    if(cfr->Visible)
	{
		cl->XData=x; //copy data into line obj
		cl->YData=y; 
		cl->ZData=z;
		line_config();
    }
...
int MatPlot::plot(const dvec& y)
{
    int n=y.size();
    dvec x; //local vector cannot pass reference to others
    x.resize(n);    
    for(int i=0;i<n;++i){x[i]=1.0*i/(n-1);}
    return line(x,y); 
}

int MatPlot::plot(const dvec& x,const dvec& y)
{	
    return line(x,y);
}

int MatPlot::plot(const valarray<double>& x,const valarray<double>& y)
{	
    dvec xx,yy;
...
int MatPlot::semilogx(const dvec& x,const dvec& y)
{
    ca->XScale=1;
    int h=line();
    if(cfr->Visible)
...
int MatPlot::semilogy(const dvec& x,const dvec& y)
{
    ca->YScale=1;    
    int h=line();
    if(cfr->Visible)
...
int MatPlot::loglog(const dvec& x,const dvec& y)
{
    ca->XScale=1;
    ca->YScale=1;
    int h=line();
...
/// errorbar: ep/em: low/upper range 
...
int MatPlot::errorbar(const dvec& x,const dvec& y,const dvec& e)
{	
	begin();	
...
int MatPlot::errorbar(const dvec& x,const dvec& y,const dvec& ep,const dvec& em)
{
	begin();
...
int MatPlot::plot3(const dvec& x,const dvec& y,const dvec& z)
{
    ca->View=1;
    begin();
...
void MatPlot::display_line()
{
    ...
    //gl2psLineWidth(cl->LineWidth);//by sguo
    //gl2psPointSize(cl->LineWidth); //by sguo
    // 2D //
    if(ca->View==0)
	{
	...
	void MatPlot::surface_config()
	{
		...
		//if( (cs->CData.size()==0) && (cs->CDataIndex.size()) )
		{
			//vector<float> rgb;
			//tcmat cdata(ny,nx);
			//tcmat cdata(ny, vector<vector<float> >(nx, vector<float>(3)) );  //dnamically create cause time!
			//tcmat cdata(ny);
			
			for(int i=0;i<ny;++i)
			{
				//cdata[i].resize(nx); //resize may consume much time
				for(int j=0;j<nx;++j)
				{
					cs->CData[i][j]=map2color(cs->CDataIndex[i][j],ca->CLim[0],ca->CLim[1]);
					//=rgb;
				}
			}
			//cs->CData=cdata;
	...
	/// create surface
	int MatPlot::surface(const dmat& Z)
	{    
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	int MatPlot::surface(const dmat& Z,const dmat& C)
	{    
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	int MatPlot::surface(const dmat& Z,const tcmat& C)
	{    
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	int MatPlot::surface(const dvec& x,const dvec& y,const dmat& Z)
	{    
		int h=surface();
		ca->View=1;
		cs->type=1;
		...
		nx=Z.size();
		ny=Z.size();
		//nz=cs->CData[0][0].size();
		if(nx!=cs->CData.size() && ny!=cs->CData[0].size())
		{
			//cs->CData.clear();
			cs->CData.resize(nx);
			for(int i=0;i<nx;i++) 
			{
				cs->CData[i].resize(ny);
				if(!nx)
				for(int j=0;j<ny;j++) 
					cs->CData[i][j].resize(3);
			}
		}
		surface_config();
		return h;
	}
	int MatPlot::surface(const dvec& x,const dvec& y,const dmat& Z,const dmat& C)
	{    
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	int MatPlot::surface(const dvec& x,const dvec& y,const dmat& Z,const tcmat& C)
	{    
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	int MatPlot::surface(const dmat& X,const dmat& Y,const dmat& Z)
	{
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	int MatPlot::surface(const dmat& X,const dmat& Y,const dmat& Z,const dmat& C)
	{
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	int MatPlot::surface(const dmat& X,const dmat& Y,const dmat& Z,const tcmat& C)
	{
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	/// surf
	int MatPlot::surf(const dvec& x, const dvec& y, const dmat& Z)
	{
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	/// create pcolor
	int MatPlot::pcolor(const dmat& C)
	{
	...
	int MatPlot::pcolor(const tcmat& C)
	{
		int h=surface();
		cs->type=0;
		cs->XData.clear();
	...
	int MatPlot::pcolor(const dvec& x, const dvec& y, const dmat& C)
	{
		int h=surface();
		cs->type=0;
		cs->XData.resize(1); cs->XData[0]=x;
	...
	int MatPlot::pcolor(const dvec& x, const dvec& y, const tcmat& C)
	{
		int h=surface();
		cs->type=0;
		cs->XData.resize(1); cs->XData[0]=x;
	...
	int MatPlot::pcolor(const dmat& X,const dmat& Y,const dmat& C)
	{
		int h=surface();
		cs->type=0; 
		cs->XData=X;
	...
	int MatPlot::pcolor(const dmat& X,const dmat& Y,const tcmat& C)
	{
		int h=surface();
		cs->type=0; 
		cs->XData=X;
	...
	/// mesh
	int MatPlot::mesh(const dvec& x, const dvec& y, const dmat& Z)
	{
		int h=surface();
		ca->View=1;
		cs->type=1;
	...
	/// contour
	int MatPlot::contour(const dmat& Z)
	{
		int h=surface();
		cs->type=3;
		cs->XData.clear();
	...
	int MatPlot::contour(const dmat& Z,int n)
	{
		int h=surface();
		cs->type=3;
		cs->XData.clear();
	...
	int MatPlot::contour(const dmat& Z, const dvec& v)
	{
		int h=surface();
		cs->type=3;
		cs->XData.clear();
	...
	int MatPlot::contour(const dvec& x,const dvec& y,const dmat& Z)
	{
		int h=surface();
		cs->type=3;
		cs->XData.resize(1); cs->XData[0]=x;
	...
	int MatPlot::contour(const dvec& x,const dvec& y,const dmat& Z,int n)
	{
		int h=surface();
		cs->type=3;
		cs->XData.resize(1); cs->XData[0]=x;
	...
	int MatPlot::contour(const dvec& x, const dvec& y, const dmat& Z, const dvec& v)
	{
		int h=surface();
		cs->type=3;
		cs->XData.resize(1); cs->XData[0]=x;
	...
	void MatPlot::display_surface_2d()
	{
		...
		if(nxi==0)
		{
			...
			if(cs->FaceColor != "none")
			{
				...
				rgb=ColorSpec2RGB(cs->FaceColor); //move here to save time
...
/// patch
int MatPlot::patch(const dmat& X,const dmat& Y)
{
    // Single color
    int h=patch();
    cp->type=0;
...
int MatPlot::patch(const dmat& X,const dmat& Y,const dvec& C)
{
...
int MatPlot::patch(const dmat& X,const dmat& Y,const tcvec& C)
{
...
int MatPlot::patch(const dmat& X,const dmat& Y,const dmat& Z)
{
    // Single color
    int h=patch(); 
    ca->View=1;
...
int MatPlot::patch(const dmat& X,const dmat& Y,const dmat& Z,const dvec& C)
{
    // One color per face    
    int h=patch();
    ca->View=1;
...
int MatPlot::patch(const dmat& X,const dmat& Y,const dmat& Z,const tcvec& C)
{
    // One color per face    
    int h=patch();    
    ca->View=1;
...
int MatPlot::bar(const dvec& y,float width)
{ 
    dvec x;
    x.resize(y.size());
    for(int i=0;i<y.size();++i)
...
int MatPlot::bar(const dvec& x,const dvec& y)
{ 
    return bar(x,y,0.8);
}

int MatPlot::bar(const dvec& x,const dvec& y,float width)
{    
    int h=patch();
    cp->type=0;
    cp->XData.clear();
...
void MatPlot::display_patch_2d()
{
    ...
    for(int i=0;i<nf;++i)
	{
		...
		if(cp->EdgeColor!="none")
		{
			...
			//gl2psLineWidth(cp->LineWidth);//by sguo
...
void MatPlot::display_text()
{
    ...
    //gl2psText(ct->String.c_str(), "Arial", 12);		 //by sguo
...
void MatPlot::ptext(float x,float y,string s)
{
	...
	else
	{
		...
		glLoadIdentity();
		pFont->Text(x,y,s.c_str(),0.02,0.02);
...
void MatPlot::ptext3(float x,float y,float z,string s)
{
	...
	else
	{
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		pFont->Text(x,y,z,s.c_str(),0.02,0.02,1);
		glMatrixMode(GL_PROJECTION);
		glPopMatrix();
	}
}  
...
void MatPlot::ptext3c(float x,float y,float z,string s)
{
	...
	else
	{
		...
		pFont->Text(x,y,z,s.c_str(),0.1,0.1,0.1);
		//glPopMatrix();
		glPopMatrix();
...
const vector<float>& MatPlot::map2color(double x,double xmin,double xmax)
{
    int n=cmap.size();
    float normx;
    //vector<float> rgb(3);
